use core::ffi::c_void;
use esp32_nimble::{ble, enums::*, utilities::voidp_to_ref, BLEAdvertisedDevice, BLEError, Signal};
use esp32_nimble::{BLEAdvertisedData, BLEDevice};
use esp_idf_svc::sys;
...
impl BLEScan {
    ...
    pub fn start<F, R>(&mut self, _ble_device: &BLEDevice, mut callback: F)
    where
        F: FnMut(&BLEAdvertisedDevice, BLEAdvertisedData<&[u8]>) -> Option<R>,
    {
        log::info!("Register BT scanner callback");
        let mut on_result =
            |scan: &mut Self, device: &BLEAdvertisedDevice, data: BLEAdvertisedData<&[u8]>| {
                log::info!("Scanner callback");
        ...
        #[cfg(esp_idf_bt_nimble_ext_adv)]
        {
            ...
            unsafe {
                ble!(sys::ble_gap_ext_disc(
                    esp32_nimble::ble_device::OWN_ADDR_TYPE as _,
                    0,
                    0,
    ...
    extern "C" fn handle_gap_event(event: *mut sys::ble_gap_event, arg: *mut c_void) -> i32 {
        ...
        match event.type_ as u32 {
            ...
            sys::BLE_GAP_EVENT_DISC_COMPLETE => (), //scan.signal.signal(()),
            _ => {}
        }
        0
    }
