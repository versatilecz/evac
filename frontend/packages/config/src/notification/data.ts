...
export const notificationConfig$ = Rx.from(service).pipe(
  Rx.map((data) => data.notification),
  Rx.shareReplay(1)
)
export const emailConfig$ = notificationConfig$.pipe(
  Rx.map((data) => data.email),
  Rx.shareReplay(1)
)
