...
import { useNotificationForm } from '@/composables'
import { ICON, $Notification, $NotificationFormData } from '@/definitions'

const props = defineProps<{ notification?: $Notification | $NotificationFormData }>()
defineOptions({ inheritAttrs: false })
const { t } = useI18n({ useScope: 'global' })
const { title, formData, hasData, hasChanges, reset, create, remove, update } = useNotificationForm(() => props.notification)
</script>
        ...
        <ContentHeader :icon="ICON">
          <template #title>
            <Dialog.Title class="headline grow">
              <input id="notification-name" v-model="formData.name" class="input" type="text" :placeholder="title" />
            </Dialog.Title>
          </template>
          <template #description>
            <Dialog.Description class="description">{{ t('notification.dialog.description', '') }}</Dialog.Description>
          </template>
          <template #row>
