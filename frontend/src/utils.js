...
export function formatDate(dateString, dateStyle = 'short', timeStyle = undefined) {
...
export function formatPrice(price) {
  const currency = new Intl.NumberFormat('cs-CZ',{style: 'currency', currency: 'CZK'})
...
export function filter(items, itemFilter) {
  ...
  if (itemFilter.tags?.length > 0) {
    items = items.filter((p) => p?.tags?.some((t) => itemFilter.tags.includes(t)))
  }
...
export function openDate(date, hourOpen) {
  ...
  if(date.getHours() <= hourOpen.hours || (date.getHours() == hourOpen.hours && date.getMinutes() <= hourOpen.minutes)) {
...
export function closeDate(date, hourClose) {
  ...
  if(date.getHours() >= hourClose.hours || (date.getHours() == hourClose.hours && date.getMinutes() >= hourClose.minutes)) {
    date.setDate(date.getDate() + 1)
  }
